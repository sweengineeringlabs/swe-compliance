...
use crate::features::reports::reports_service;
...
pub fn export(store: &ReportsStore) {
    ...
    let report_data = store.report_data;
    let loading = store.loading;
    let error = store.error;

    spawn(async move {
        match reports_service::export_report(&scan_id, &format).await {
            Ok(report) => {
...
pub fn export_audit(store: &ReportsStore) {
    ...
    spawn(async move {
        match reports_service::get_audit_report(&scan_id).await {
            Ok(report) => {
                report_data.set(Some(report));
                loading.set(false);
            }
...
pub fn compare(store: &ReportsStore) {
    ...
    spawn(async move {
        match reports_service::compare_reports(&scan_a, &scan_b).await {
            Ok(result) => {

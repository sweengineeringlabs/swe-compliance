...
/// Determine the CSS class for a compliance rate value.
fn rate_css_class(rate: f64) -> &'static str {
    if rate >= 90.0 {
...
#[component]
pub fn category_chart(
    categories: Signal<Vec<CategoryBreakdown>>,
) -> View {
    let categories_for_sort = categories.clone();
    let sorted_categories = derived(move || {
        let mut cats = categories_for_sort.get().clone();
        cats.sort_by(|a, b| a.category.cmp(&b.category));
        cats
    });

    let categories_check = categories.clone();
    let has_data = derived(move || !categories_check.get().is_empty());
                    ...
                    <tbody>
                        for cat in sorted_categories.get().iter() {
                            <tr data-testid={format!("category-row-{}", slug(&cat.category))}>
                                ...
                                <td class="category-chart__bar-cell">
                                    {
                                        ...
                                        let bar_testid = format!("category-bar-{}", slug(&cat.category));
                                        let passed_style = format!("width: {:.1}%", bar_pct(cat.passed, t));
                                        let failed_style = format!("width: {:.1}%", bar_pct(cat.failed, t));
                                        let skipped_style = format!("width: {:.1}%", bar_pct(cat.skipped, t));
                                        if t > 0 {
                                            view! {
                                                <div
                                ...
                                <td class="category-chart__td">
                                    {
                                        let rate_class = rate_css_class(cat.pass_rate());
                                        let rate_text = format!("{:.0}%", cat.pass_rate());
                                        let rate_testid = format!("category-rate-{}", slug(&cat.category));
                                        <span
                                            class={rate_class}
                                            data-testid={rate_testid}
                                        >
                                            {rate_text}
                                        </span>
                                    }

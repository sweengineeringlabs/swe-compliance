use rsc_ui::prelude::*;
use crate::features::struct_engine::struct_engine_type::{StructCheck, CrateNode};
use crate::features::struct_engine::struct_engine_service;
...
static STORE: StructEngineStore = StructEngineStore::new();

/// Public accessor signals.
pub static checks: Signal<Vec<StructCheck>> = STORE.checks;
pub static crate_tree: Signal<Option<CrateNode>> = STORE.crate_tree;
pub static project_kind: Signal<Option<String>> = STORE.project_kind;
pub static category_filter: Signal<Option<String>> = STORE.category_filter;
pub static status_filter: Signal<Option<String>> = STORE.status_filter;
pub static loading: Signal<bool> = STORE.loading;
pub static error: Signal<Option<String>> = STORE.error;
...
pub fn load_results(scan_id: &str) {
    loading.set(true);
    error.set(None);
    ...
    spawn(async move {
        match struct_engine_service::get_struct_results(&scan_id_owned).await {
            Ok(results) => {
                checks.set(results);
                loading.set(false);
            }
...
pub fn load_crate_layout(project_id: &str) {
    loading.set(true);
    error.set(None);
    ...
    spawn(async move {
        match struct_engine_service::get_crate_layout(&project_id_owned).await {
            Ok(tree) => {
...
pub fn load_project_kind(project_id: &str) {
    let project_id_owned = project_id.to_string();

    spawn(async move {
        match struct_engine_service::get_project_kind(&project_id_owned).await {
            Ok(kind) => {
...
/// Set category filter.
pub fn set_category_filter(value: Option<String>) {
    category_filter.set(value);
}

/// Set status filter.
pub fn set_status_filter(value: Option<String>) {

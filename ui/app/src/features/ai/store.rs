...
pub fn load_status(store: &AiStore) {
    ...
    spawn(async move {
        match service::get_ai_status().await {
            Ok(ai_status) => {
...
pub fn send_message(store: &AiStore) {
    ...
    spawn(async move {
        match service::send_chat(&text).await {
            Ok(reply) => {
...
pub fn run_audit(store: &AiStore, path: &str, scope: &str) {
    ...
    let audit_result = store.audit_result.clone();
    let loading = store.loading.clone();
    let error = store.error.clone();
    ...
    spawn(async move {
        match service::run_audit(&path_owned, &scope_owned).await {
            Ok(result) => {
...
pub fn generate_commands(store: &AiStore, requirements_json: &str, project_context: &str) {
    ...
    let command_result = store.command_result.clone();
    let loading = store.loading.clone();
    let error = store.error.clone();
    ...
    spawn(async move {
        match service::generate_commands(&reqs_owned, &ctx_owned).await {
            Ok(result) => {

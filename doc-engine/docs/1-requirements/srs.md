...
| **Audit report (IEEE 1028)** | A hand-written markdown document at `docs/2-planning/audit_report.md` containing Scope, Findings, and Recommendations sections per IEEE 1028:2008 clause 4; verified by checks 123 and 124. This is a human-authored artifact — not generated by doc-engine |
| **Audit status report (ISO 15289)** | A machine-generated JSON information item conforming to ISO/IEC/IEEE 15289:2019 clause 9.2; persisted via `doc-engine scan --output <path>` to `docs/7-operations/compliance/documentation_audit_report_v{version}.json`; contains identification metadata (standard, clause, tool, version, timestamp), scope (project_root, project_type, project_scope), and per-check results with summary. This is tool output — not hand-written |
| **Project scope** | Tier that determines which rule subset applies: Small (day-one essentials), Medium (team collaboration and traceability), Large (full ISO/IEC/IEEE compliance). See FR-505a for tier criteria |
...
| ISO/IEC/IEEE 15289:2019 | Content of life-cycle information items (checks 1-8, 14-20, 26-39, 48-50, 55, 60-68, 69-76, 99, 102-128); clause 9.2 defines the "Audit Report" information item — doc-engine persists its machine-generated scan output as `docs/7-operations/compliance/documentation_audit_report_v{version}.json`; the separate hand-written IEEE 1028 audit report lives at `docs/2-planning/audit_report.md` |
...
| **State** | Implemented |
| **Verification** | Test |
| **Traces to** | STK-01, STK-05 -> `rules.toml`, `core/engine.rs` |
| **Acceptance** | A rule with `depends_on = [N]` is automatically skipped (with reason "dependency check N failed") when any of its parent checks produced a Fail result; parent checks always execute before their dependents; the dependency graph is acyclic |

Rules shall support an optional `depends_on` field — an array of check IDs that must pass before this check executes. When any parent check fails, the dependent check shall be skipped with reason `"Skipped: dependency check {id} failed"` and counted toward the `skipped` total. The engine shall resolve the dependency graph before execution and reject cyclic dependencies at load time.

**Rationale**: Content validation, naming convention, and cross-reference checks are meaningless when the target file does not exist. Without dependency tracking, these checks produce misleading "file not found" violations that obscure the root cause (the missing file) behind multiple redundant failures. The dependency graph also enables consolidation of duplicate existence checks — currently 5 files (README.md, CONTRIBUTING.md, CHANGELOG.md, SECURITY.md, LICENSE) are each checked for existence in two separate categories (naming and root_files), producing duplicate failures when absent.

**Example `rules.toml` usage**:
```toml
# Parent: file existence check
[[rules]]
id = 26
category = "root_files"
type = "file_exists"
...
# Child: content validation — only runs if check 26 passes
[[rules]]
id = 40
category = "navigation"
...
**Known dependency pairs** (illustrative, not exhaustive):

| Parent (existence) | Child (content/naming) | File |
|---------------------|----------------------|------|
| 26 | 14, 40 | README.md |
...
#### FR-306: Duplicate check consolidation
...
| **State** | Implemented |
| **Verification** | Inspection |
| **Traces to** | FR-305, STK-01 -> `rules.toml` |
| **Acceptance** | No two rules in `rules.toml` verify existence of the same file path; formerly-duplicate naming checks (14-18) are converted to naming-convention checks or removed, with their existence semantics consolidated into root_files checks (26-30) |

Once FR-305 (dependency graph) is implemented, the 5 duplicate existence check pairs shall be consolidated. The `root_files` category (checks 26-30) shall own file existence; the `naming` category (checks 14-18) shall either validate naming conventions (e.g., uppercase, no extension for LICENSE) or be removed if the naming convention is already implicit. Content and navigation checks that depend on these files shall declare `depends_on` pointing to the root_files existence check.
...
| **Acceptance** | The scan command always persists a JSON audit report. Default path: `docs/7-operations/compliance/documentation_audit_report_v{version}.json` (relative to the scanned project root). `--output <path>` overrides the default. Parent directories are created automatically. The filename follows ISO/IEC/IEEE 15289:2019 naming. |

The scan command always persists the audit status report as JSON. When `--output` / `-o` is provided, the engine writes to that path. When omitted, it defaults to `{project_root}/docs/7-operations/compliance/documentation_audit_report_v{tool_version}.json`. Parent directories are created automatically. Stdout output is unaffected. If the file cannot be written, the tool exits with code 2.
...
| **Acceptance** | The scan command always persists a JSON audit status report conforming to ISO/IEC/IEEE 15289:2019 clause 9.2 to `docs/7-operations/compliance/documentation_audit_report_v{version}.json` (or an `--output` override); the report contains: standard, clause, tool, tool_version, timestamp (ISO 8601 UTC), project_root (absolute path), project_type, project_scope, results, summary |

The scan command always serializes an `AuditStatusReport` as pretty-printed JSON. The default output path is `{project_root}/docs/7-operations/compliance/documentation_audit_report_v{tool_version}.json`; `--output <path>` overrides this. The report wraps `ScanReport` fields (promoted to top level) with ISO 15289:2019 clause 9.2 identification metadata: standard identifier, clause reference, tool name, tool version (from `Cargo.toml`), ISO 8601 UTC timestamp, and canonicalized project root path. The `--json` stdout output is unaffected and continues to emit the bare `ScanReport`. No new dependencies are introduced.
...
| **State** | Implemented |
| **Verification** | Inspection |
| **Traces to** | STK-10 -> `config/rules.toml` |
| **Acceptance** | Each rule's `scope` value matches its tier as determined by the criteria below. A rule SHALL be `small` if it satisfies any Small criterion (1-5); `medium` if it satisfies any Medium criterion (1-6) but no Small criterion; `large` if it satisfies any Large criterion (1-7) but no Small or Medium criterion. Any scope change to `rules.toml` requires updating this policy. |

Scope tiers are cumulative — Medium includes all Small checks, Large includes all Medium checks.

**Small — Day-one essentials (21 checks)**

A project at Small scope has the minimum documentation needed to be found, understood, and contributed to. Criteria for a check to be Small:

1. **Discoverability**: The project can be found and its purpose understood (README, LICENSE, docs/ hub, glossary)
2. **Navigability**: Documentation structure is consistent and links work (SDLC numbering, cross-references, navigation from root to hub)
3. **Consistency**: Naming conventions are enforced (lowercase, underscores, no spaces, `docs/` not `doc/`)
4. **Core SDLC artifacts**: The four non-negotiable documents exist — architecture (how it's built), developer guide (how to work on it), testing strategy (how it's verified), and test plan (what's tested)
5. **Audience declaration**: Every document states who it's for

**Medium — Team collaboration and traceability (+42 = 63 checks)**

A project at Medium scope has documentation needed for multi-person teams, security practices, and decision tracking. Criteria for a check to be Medium (beyond Small):

1. **Collaboration files**: CONTRIBUTING, CHANGELOG, SECURITY, CODE_OF_CONDUCT, GitHub templates
2. **Content quality**: TLDR sections for long documents, glossary formatting and alphabetization
3. **Decision records**: ADR directory, ADR index, ADR naming conventions
4. **SDLC phase coverage**: All 8 phases have their expected artifacts (ideation through operations)
5. **Traceability**: Design documents reference requirements, planning references architecture, backlog references SRS
6. **Module maturity**: Module READMEs follow W3H, modules have examples and integration tests

**Large — Full ISO/IEC/IEEE compliance (+51 = 128 checks)**

A project at Large scope meets the complete documentation requirements of the referenced standards. Criteria for a check to be Large (beyond Medium):

1. **ISO 29148:2018**: Requirement attribute tables in SRS
2. **ISO 42010:2022**: Architecture document sections (viewpoints, concerns, rationale)
3. **ISO 29119-3:2021**: Test plan, test design, test case, and verification report section validation
4. **ISO 25010:2023**: Quality model sections in production readiness
5. **ISO 12207:2017**: Lifecycle process sections
6. **Supplementary artifacts**: Templates directory, compliance checklist, ConOps, StRS, traceability matrix, progress reports, decision log
7. **Extended operations**: Operations manual, troubleshooting guide, maintenance plan, disposal plan

**Scope decision tree**

When adding or reclassifying a rule, walk this tree top-down. The first YES determines the scope.

```
1. Can a solo developer on day one NOT ship without this?
   │
   ├─ YES: Is it about finding/understanding the project?
   │       (README, LICENSE, docs/ hub, glossary, audience)
   │       → scope = "small"  (Discoverability / Audience)
   │
   ├─ YES: Is it about links working and names being consistent?
   │       (cross-refs resolve, lowercase, underscores, docs/ not doc/)
   │       → scope = "small"  (Navigability / Consistency)
   │
   ├─ YES: Is it architecture, developer guide, testing strategy, or test plan?
   │       → scope = "small"  (Core SDLC artifacts)
   │
   └─ NO: move to step 2.

2. Does a team need this for collaboration, security, or traceability?
   │
   ├─ YES: Is it a collaboration file?
   │       (CONTRIBUTING, CHANGELOG, SECURITY, CODE_OF_CONDUCT, templates)
   │       → scope = "medium"
   │
   ├─ YES: Is it content quality, ADRs, or module maturity?
   │       (TLDR, glossary format, ADR index, module W3H/examples/tests)
   │       → scope = "medium"
   │
   ├─ YES: Is it an SDLC phase artifact or traceability link?
   │       (phase index, design→requirements, planning→architecture)
   │       → scope = "medium"
   │
   └─ NO: move to step 3.

3. Is it ISO/IEC/IEEE standards content validation or a supplementary artifact?
   │
   ├─ YES → scope = "large"
   │
   └─ NO: re-evaluate — the check may not be needed.
```
...
| **Verification** | Test |
| **Traces to** | STK-09 -> `main.rs` |
| **Acceptance** | The scan command always persists a JSON report to `docs/7-operations/compliance/documentation_audit_report_v{version}.json` by default; `--output <path>` or `-o <path>` overrides the default path; parent directories are created automatically |
...
Severity mapping: mandatory artifacts (per ISO) use `warning`; recommended artifacts use `info`. Checks 66, 68, 83, 88, 100, 101 are `warning` (matching their mandatory ISO status). Scope mapping: per FR-505a tier criteria.
...
| 9.2 | Audit report | 123, 124, FR-403, FR-831 | **Two distinct artifacts**: (1) `docs/2-planning/audit_report.md` — hand-written IEEE 1028 findings document (checks 123, 124); (2) `docs/7-operations/compliance/documentation_audit_report_v{version}.json` — machine-generated scan status report persisted via `--output` (FR-403, FR-831) |

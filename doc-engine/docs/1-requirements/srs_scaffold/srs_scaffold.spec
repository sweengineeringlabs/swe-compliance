...
|-----|--------|-------|----------|--------------|------------|
| REQ-001 | FR-822 | Scaffold command | Should | Demonstration | `doc-engine scaffold <SRS_PATH> [--output DIR] [--force] [--phase PHASES]` parses the SRS, extracts domains and requirements, and generates per-domain SDLC spec files; `--phase` accepts comma-separated phase names to filter output; exit code 0 on success, 2 on error |
...
| REQ-008 | FR-829 | Scaffold phase filter | Should | Test | `--phase` accepts a comma-separated list of SDLC phases (`requirements`, `design`, `testing`, `deployment`); only files for the specified phases are generated; BRD files are included only when `requirements` is selected; omitting `--phase` generates all phases; invalid phase names exit with code 2 and a descriptive error; phase names are case-insensitive |
| REQ-009 | FR-830 | Scaffold status report (ISO 15289 clause 9) | Should | Test | `--report <path>` persists a JSON scaffold status report conforming to ISO/IEC/IEEE 15289:2019 clause 9; the report contains: standard, clause, tool, tool_version, timestamp (ISO 8601 UTC), srs_source (absolute path), phases, force, domain_count, requirement_count, created, skipped |

## Acceptance Criteria

- **REQ-001** (FR-822): `doc-engine scaffold <SRS_PATH> [--output DIR] [--force] [--phase PHASES]` parses the SRS, extracts domains and requirements, and generates per-domain SDLC spec files; `--phase` accepts comma-separated phase names to filter output; exit code 0 on success, 2 on error
...
- **REQ-008** (FR-829): `--phase` accepts a comma-separated list of SDLC phases (`requirements`, `design`, `testing`, `deployment`); only files for the specified phases are generated; BRD files are included only when `requirements` is selected; omitting `--phase` generates all phases; invalid phase names exit with code 2 and a descriptive error; phase names are case-insensitive
- **REQ-009** (FR-830): `--report <path>` persists a JSON scaffold status report conforming to ISO/IEC/IEEE 15289:2019 clause 9; the report contains: standard, clause, tool, tool_version, timestamp (ISO 8601 UTC), srs_source (absolute path), phases, force, domain_count, requirement_count, created, skipped

...
mod tests {
    use super::*;
    use crate::api::types::{
        CheckEntry, CheckId, CheckResult, ProjectScope, ProjectType, ScanSummary,
    };
    ...
    fn test_file_sink_creates_file() {
        let dir = std::env::temp_dir().join("doc_engine_sink_test_creates");
    ...
    fn test_file_sink_creates_parent_dirs() {
        let dir = std::env::temp_dir().join("doc_engine_sink_test_parents");
        let _ = std::fs::remove_dir_all(&dir);
    ...
    fn test_file_sink_roundtrip() {
        let dir = std::env::temp_dir().join("doc_engine_sink_test_roundtrip");
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("report.json");
